 ...
 *
 * <p>Before you write your own lookup strategy, know that:
 *
 * <ul>
 ...
 *   only influences the so-called source name of the template ({@link Template#getSourceName()}). The template's
 *   name is
 *   used as the basis for resolving relative inclusions/imports in the template. The source name is pretty much only
 *  used
 *   in error messages as error location, and of course, to actually load the template "file".
 *
 *   <li>Understand the impact of the last point if your template lookup strategy fiddles not only with the file name
 *   part of the template name, but also with the directory part. For example, one may want to map "foo.ftl" to
 *   "en/foo.ftl", "fr/foo.ftl", etc. That's legal, but the result is kind of like if you had several root
 *   directories ("en/", "fr/", etc.) that are layered over each other to form a single merged directory. (This is
 *   what's desirable in typical applications, yet it can be confusing.)
 * </ul>
 *
...
public abstract class TemplateLookupStrategy {
     ...
     *
     * <p>Through an example: Assuming localized lookup is enabled and that a template is requested for the name
     ...
     *
     * <p>The generation of the localized name variation with the default lookup strategy, happens like this: It
     * removes the file extension (the part starting with the <em>last</em> dot), then appends {@link Locale#toString()}
     * after it, and puts back the extension. Then it starts to remove the parts from the end of the locale, considering
     ...
     *
     * @param ctx
     *         Contains the parameters for which the matching template need to be found, and operations that are needed
     *         to implement the strategy. Some of the important input parameters are:
     *         {@link TemplateLookupContext#getTemplateName()}, {@link TemplateLookupContext#getTemplateLocale()}. The
     *         most important operations are {@link TemplateLookupContext#lookupWithAcquisitionStrategy(String)} and
     *         {@link TemplateLookupContext#createNegativeLookupResult()}. (Note that you deliberately can't use
     *         {@link TemplateLoader}-s directly to implement lookup.)
     *
