 ...
 * provides a few {@link TemplateLoader} implementations out-of-the-box, it's normal for embedding frameworks to use
...
public interface TemplateLoader {
     ...
     *
     * @param name
     *         The name (template root directory relative path) of the template, already localized and normalized by the
     ...
     *         separated by a slash (not backslash). Backslashes (or any other OS specific separator character) are not
     *         considered as separators by FreeMarker, and thus they will not be replaced with slash before passing to
     *         this method, so it's up to the template loader to handle them (say, by throwing an exception that tells
     *         the user that the path (s)he has entered is invalid, as (s)he must use slash -- typical mistake of
     *         Windows users). The passed names are always considered relative to some loader-defined root location
     *         (often referred as the "template root directory"), and will never start with a slash, nor will they
     *         contain a path component consisting of either a single or a double dot -- these are all resolved by the
     *         template cache before passing the name to the loader. As a side effect, paths that trivially reach
     *         outside template root directory, such as {@code ../my.ftl}, will be rejected by the template cache, so
     *         they never reach the template loader. Note again, that if the path uses backslash as path separator
     *         instead of slash as (the template loader should not accept that), the normalization will not properly
     *         happen, as FreeMarker (the cache) recognizes only the slashes as separators.
     *
     * @return An object representing the template source, which can be supplied in subsequent calls to
     * {@link #getLastModified(Object)} and {@link #getReader(Object, String)}, when those are called on the same
     * {@link TemplateLoader}. {@code null} must be returned if the source for the template doesn't exist; don't throw
     * exception then! The exact type of this object is up to the {@link TemplateLoader} implementation. As this object
     * is possibly used as hash key in caches, and is surly compared with another template source for equality, <b>it
     * must have a proper {@link Object#equals(Object)} and {@link Object#hashCode()}) implementation</b>. Especially,
     * template sources that refer to the same physical source must be equivalent, otherwise template caching can become
     * inefficient. This is only expected from {@link Object#equals(Object)} when the compared template sources came
     * from the same {@link TemplateLoader} instance. Also, it must not influence the equality if the source is open or
     ...
     *         existing template. Don't throw exception if the template doesn't exist, instead return with {@code null}
     *         then!
     */
     ...
     * value meant to be milliseconds since the epoch, but in fact FreeMarker doesn't care what it means, it only cares
     * if it changes, in which case the template needs to be reloaded (even if the value has decreased). -1 is not
     * special in that regard either; if you keep returning it, FreeMarker won't reload the template (as far as it's not
     * evicted from the cache from some other reason). Note that {@link Long#MIN_VALUE} is reserved for internal use.
     */
     ...
     *
     * <p>
     * Unlike {@link #findTemplateSource(String)}, this method must not tolerate if the template is not found, and must
     * throw {@link IOException} in that case.
     *
     * @param templateSource
     ...
     *         {@link TemplateLoader#closeTemplateSource(Object)} wasn't applied yet.
     * @param encoding
     *         the character encoding used to translate source bytes to characters. Some loaders may not have access to
     *         the byte representation of the template stream, and instead directly obtain a character stream. These
     ...
     * the caller (which is {@link TemplateCache} usually) to {@code close()} it. The {@link Reader} is not required to
     * work after the {@code templateSource} was closed ({@link #closeTemplateSource(Object)}).
     *
     * @throws IOException
