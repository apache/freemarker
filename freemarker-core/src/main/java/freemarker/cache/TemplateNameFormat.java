...
public abstract class TemplateNameFormat {
     ...
     *
     * <p>Differences to the {@link #DEFAULT_2_3_0} format:
     *
     * <ul>
     *
     ...
     *
     * @param baseName
     *         Maybe a file name, maybe a directory name. The meaning of file name VS directory name depends on the name
     *         format, but typically, something like "foo/bar/" is a directory name, and something like "foo/bar" is a
     *         file name, and thus in the last case the effective base is "foo/" (i.e., the directory that contains the
     *         file). Not {@code null}.
     * @param targetName
     *         The name to convert. This usually comes from a template that refers to another template by name. It can
     *         be a relative name, or an absolute name. (In typical name formats absolute names start with {@code "/"}
     *         or maybe with an URL scheme, and all others are relative). Not {@code null}.
     *
     ...
     * templates). The standard implementations shipped with FreeMarker always return a root relative path (except if
     * the name starts with an URI schema, in which case a full URI is returned).
     */
