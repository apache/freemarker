...
/**
 * Performs caching and on-demand loading of the templates. The actual template "file" loading is delegated to a
 * {@link TemplateLoader} that you can specify in the constructor. Some aspects of caching is delegated to a
 * {@link CacheStorage} that you can also specify in the constructor.
 *
 ...
 * {@link Configuration#setTemplateLoader(TemplateLoader)} and {@link Configuration#setCacheStorage(CacheStorage)}.
 */
public class TemplateCache {
    ...
    private static final String ASTERISK_STR = String.valueOf(ASTERISK);
    private static final char SLASH = '/';
    private static final char LOCALE_INFIX_SEPARATOR = '_';
    ...
    /**
     * Returns a template cache that will first try to load a template from the file system relative to the current user
     * directory (i.e. the value of the system property <code>user.dir</code>), then from the classpath.
     *
     ...
     * @param config
     *         The {@link Configuration} this cache will be used for. Can be {@code null} for backward compatibility, as
     *         it can be set with {@link #setConfiguration(Configuration)} later.
     *
     * @since 2.3.24
     */
    public TemplateCache(TemplateLoader templateLoader, CacheStorage cacheStorage,
    ...
    /**
     * Sets the configuration object to which this cache belongs. This method is called by the configuration itself to
     * establish the relation, and should not be called by users.
     *
     ...
     * {@link MaybeMissingTemplate} object that contains {@code null} as the {@link Template} and information about the
     * missing template. The return value itself is never {@code null}. Note that exceptions occurring during template
     * loading will not be classified as a missing template, so they will cause an exception to be thrown by this method
     * instead of returning a {@link MaybeMissingTemplate}. The idea is that having a missing template is normal (not
     * exceptional), providing that the backing storage mechanism could indeed check that it's missing.
     *
     ...
     * @throws IOException
     *         If reading the template has failed from a reason other than the template is missing. This method should
     *         never be a {@link TemplateNotFoundException}, as that condition is indicated in the return value.
     * @since 2.3.22
     */
    ...
    private Template loadTemplate(
        ...
        try {
            ...
            throw new IOException("Error while getting TemplateConfiguration; see cause exception.", e);
        }
        if (tc != null) {
     ...
     * {@link #getTemplate(String, Locale, String, boolean)} calls. If the configured template loader is
     * {@link StatefulTemplateLoader stateful}, then its {@link StatefulTemplateLoader#resetState()} method is invoked
     * as well.
     */
    public void clear() {
        synchronized (storage) {
     ...
     *         If the {@code baseName} or {@code targetName} is malformed according the {@link TemplateNameFormat} in
     *         use.
    ...
    private TemplateLookupResult lookupTemplateWithAcquisitionStrategy(String path) throws IOException {
        ...
        int lastAsterisk = -1;
        List<String> tokpath = new ArrayList<>();
        while (tok.hasMoreTokens()) {
            String pathToken = tok.nextToken();
            if (pathToken.equals(ASTERISK_STR)) {
                if (lastAsterisk != -1) {
    ...
    private String concatPath(List<String> path, int from, int to) {
    ...
    /**
     * This class holds a (name, locale) pair and is used as the key in the cached templates map.
     */
            ...
            if (o instanceof TemplateKey) {
                ...
                return
                        parse == tk.parse &&
                                ...
                                Objects.equals(customLookupCondition, tk.customLookupCondition) &&
    ...
    /**
     * This class holds the cached template and associated information (the source object, and the last-checked and
     * last-modified timestamps). It is used as the value in the cached templates map. Note: this class is Serializable
     * to allow custom 3rd party CacheStorage implementations to serialize/replicate them (see tracker issue #1926150);
     * FreeMarker code itself doesn't rely on its serializability.
     */
    ...
    private class TemplateCacheTemplateLookupContext extends TemplateLookupContext {
        ...
        public TemplateLookupResult lookupWithLocalizedThenAcquisitionStrategy(final String templateName,
            ...
            String localeInfix = toLocaleInfix(templateLocale);
            StringBuilder buf = new StringBuilder(templateName.length() + localeInfix.length());
            buf.append(prefix);
            while (true) {
                buf.setLength(prefix.length());
                String path = buf.append(localeInfix).append(suffix).toString();
                TemplateLookupResult lookupResult = lookupWithAcquisitionStrategy(path);
                if (lookupResult.isPositive()) {
                    return lookupResult;
                }

                int lastUnderscore = localeInfix.lastIndexOf(LOCALE_INFIX_SEPARATOR);
                if (lastUnderscore == -1) {
                    break;
                }
                localeInfix = localeInfix.substring(0, lastUnderscore);
            }
    ...
    /**
     * Originally FreeMarker relied on {@link Locale#toString()}, but that's actually fragile, so we recreate it as it
     * was in older Java, but we leave out extensions (no needed for lookup), and  also add the initial infix
     * separator.
     */
    private static String toLocaleInfix(Locale locale) {
        boolean hasLang = !locale.getLanguage().isEmpty();
        ...
        appendPartToLocaleInfix(suffix, locale.getLanguage(), 0);

        if (hasCountry || (hasLang && (hasVariant || hasScript))) {
            appendPartToLocaleInfix(suffix, locale.getCountry(), 1);
        }
        if (hasVariant && (hasLang || hasCountry)) {
            appendPartToLocaleInfix(suffix, locale.getVariant(), 2);
        }

        if (hasScript && (hasLang || hasCountry)) {
            appendPartToLocaleInfix(suffix, locale.getScript(), 3);
        }

        return suffix.toString();
    }

    private static void appendPartToLocaleInfix(StringBuilder suffix, String part, int partIndex) {
        suffix.append(LOCALE_INFIX_SEPARATOR);
        if (partIndex == 3) { // 3 => script
            suffix.append('#');
        }
        for (int i = 0; i < part.length(); i++) {
            char c = part.charAt(i);
            if (localePartCloser(c)) {
                break;
            }
            suffix.append(c);
        }
    ...
    private static boolean localePartCloser(char c) {
        return c < 0x20 || c == '%' || c == '/' || c == '\\';
    }

    /**
     * Return value of {@link TemplateCache#getTemplate(String, Locale, Object, String, boolean)}.
     *
     * @since 2.3.22
     */
    public final static class MaybeMissingTemplate {
        ...
        private MaybeMissingTemplate(
                String normalizedName,
                MalformedTemplateNameException missingTemplateCauseException) {
            this.template = null;
